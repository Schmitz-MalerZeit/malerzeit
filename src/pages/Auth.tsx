    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-10">
      <div className="w-full max-w-sm">
        <div className="flex flex-col items-center mb-8">
          <Logo size="lg" />
          <p className="text-sm text-muted-foreground mt-3">Professionelle Preisvorschläge in Minuten</p>
        </div>
        ...
        <div className="bg-card rounded-2xl shadow-elevated p-6 border border-border">
          ...
          <div className="relative my-5">
            ...
            <div className="relative flex justify-center"><span className="bg-card px-3 text-xs text-muted-foreground uppercase tracking-wider">oder</span></div>
          </div>
          ...
          <form onSubmit={handleEmail} className="space-y-4">
            ...
            <Button type="submit" disabled={loading} className="w-full h-12 text-base font-semibold gradient-primary text-primary-foreground border-0 shadow-soft hover:shadow-glow transition-base">
              {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><Mail className="h-4 w-4 mr-2" /> {mode === "signin" ? "Anmelden" : "Konto erstellen"}</>}
            </Button>
          </form>
        </div>
